...
use crate::{
    crypto, error::ServiceError, models::candidate::ApplicationDetails,
    utils::date::parse_naive_date_from_opt_str,
};

use super::{
    candidate::{CandidateDetails, ParentDetails},
    grade::GradeList,
    school::School,
};
...
impl EncryptedCandidateDetails {
    pub async fn new(
        ...
        let (first_school_str, second_school_str) = (
            form.first_school.to_string(),
            form.second_school.to_string(),
        );
...
impl From<&candidate::Model> for EncryptedCandidateDetails {
    fn from(candidate: &candidate::Model) -> Self {
        EncryptedCandidateDetails {
            ...
            personal_id_number: Some(EncryptedString::from(
                candidate.personal_identification_number.to_owned(),
            )),
...
impl EncryptedApplicationDetails {
    ...
    pub async fn decrypt(self, priv_key: String) -> Result<ApplicationDetails, ServiceError> {
        ...
        let decrypted_parents =
            future::try_join_all(self.parents.iter().map(|d| d.decrypt(&priv_key))).await?;
    ...
    pub fn is_filled(&self) -> bool {
        self.candidate.is_filled() && self.parents.iter().all(|p| p.is_filled())
    }
}

impl From<(&candidate::Model, &Vec<parent::Model>)> for EncryptedApplicationDetails {
    fn from((candidate, parents): (&candidate::Model, &Vec<parent::Model>)) -> Self {
        let enc_parents = parents
            .iter()
...
pub mod tests {
    ...
    use crate::{
        ...
        services::candidate_service::tests::put_user_data,
    ...
    async fn test_encrypted_application_details_from_candidate_parent() {
        ...
        let encrypted_details =
            EncryptedApplicationDetails::try_from((&candidate, &parents)).unwrap();
